...
public class Dynamic extends ComputerPlayer {
    PlayerNumber mynum;
    Node head;
    HashMap<Game,Node> map = new HashMap<Game,Node>();
	public Dynamic() { super("Dynamic Algorithm"); }
    ...
    private void createTreeRecur(Node node) throws GameException{
        if (node.game.isGameOver() || map.containsKey(node.game))
            return;
        map.put(node.game,node);
